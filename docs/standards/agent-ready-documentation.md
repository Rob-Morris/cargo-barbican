...
- Notes any out-of-repo context that is important to the project - but does not depend on it for completeness (see [External context](#external-context))
...
- Names the layer in one line
- States that layer's perspective and purpose
- Lists each artefact in the layer with a one-line summary and link

The index is the stable routing surface; artefacts beneath it may change more frequently.

**The root docs index - `docs/README.md`.** The repo-level documentation router. It links to every layer index and any shared standards index, and names convention-based exceptions at their chosen locations.

## Convention-based exceptions

Some artefacts live at conventional ecosystem locations rather than inside a layer subtree. These are not escapes from the model; they are part of it and are indexed explicitly.
...
**CHANGELOG.md** *(descriptive)*. If a project keeps a changelog at a conventional location, it is indexed from the bootstrap or root docs index as a convention-based exception. The standard does not prescribe a changelog format.

**SECURITY.md** *(descriptive)*. If present, the bootstrap names it as a convention-based exception so agents can find the project's security reporting or policy surface directly.

Projects may define additional convention-based exceptions when a widely recognised ecosystem location is materially useful. The rule is explicit indexing: no hidden side channels.

## Decision records

Architectural decisions are documented as **decision records** (DDs). They preserve the reasoning behind non-obvious choices so later contributors can recover context without reconstructing it from old diffs.
...
**Body is immutable.** A decision record freezes the decision's reasoning as written at the time. The record's Context, Decision, Alternatives, and Consequences sections are not rewritten later to match current reality. Current-state docs carry today's behaviour. The DD carries the historical why. That separation is deliberate; immutability is what makes a decision record a decision record rather than a living doc.
